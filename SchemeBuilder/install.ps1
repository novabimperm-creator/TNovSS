...
$project = Join-Path $root 'src\SchemeBuilder\SchemeBuilder.csproj'
...
$targetDir = Join-Path $addinsDir 'SchemeBuilder'
...
$binDir = Join-Path $root "src\SchemeBuilder\bin\$Configuration"
...
Copy-Item (Join-Path $binDir 'SchemeBuilder.dll') $targetDir -Force
$pdb = Join-Path $binDir 'SchemeBuilder.pdb'
if (Test-Path $pdb) { Copy-Item $pdb $targetDir -Force }

Copy-Item (Join-Path $root 'src\SchemeBuilder\SchemeBuilder.addin') $addinsDir -Force

Write-Host ''
Write-Host 'Установлено:' -ForegroundColor Green
Write-Host "  манифест : $(Join-Path $addinsDir 'SchemeBuilder.addin')"
Write-Host "  сборка   : $(Join-Path $targetDir 'SchemeBuilder.dll')"
Write-Host ''
Write-Host "Запустите Revit $RevitVersion — на ленте появится вкладка «Структурная схема»."
